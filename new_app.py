...
def filter_out_unicode_responses(df):
    """
    Drops entire rows where any string column contains non-ASCII characters.
    This safely removes non-English participants from the denominator entirely.
    """
    ...
    for col in df.columns:
        if df[col].dtype == object:
            # Keep the row only if the cell is NaN or contains pure ASCII
            df = df[df[col].apply(lambda x: pd.isna(x) or all(ord(c) < 128 for c in str(x)))]
    return df
...
def build_dynamic_answer_mapping(df, base_answer_key, current_api_key):
    ...
    Below is a JSON object mapping questions to their "correct_concept" in English, along with a list of raw "user_responses" (which may include typos or alternate phrasings).
...
if st.session_state.get('process_clicked', False) and (pre_url or post_url):
    ...
    with st.spinner("Fetching data from Google Sheets and cleaning Unicode..."):
        if pre_url: 
            pre_df = fetch_google_sheet_data(pre_url)
            pre_df = filter_out_unicode_responses(pre_df) # Drop non-English rows
        if post_url: 
            post_df = fetch_google_sheet_data(post_url)
            post_df = filter_out_unicode_responses(post_df) # Drop non-English rows
        
    if (pre_df is not None and not pre_df.empty) or (post_df is not None and not post_df.empty):
        with st.spinner("Analyzing responses and mapping English typos to the answer key (this takes 10-15 seconds)..."):
        ...
        if pre_df is not None and not pre_df.empty:
            with cols[0]:
                ...
                c1.metric("Clean Data Participants", len(pre_scores))
        ...
        if post_df is not None and not post_df.empty:
            with cols[1] if pre_df is not None and not pre_df.empty else cols[0]:
                st.markdown("**Post-Webinar Statistics**")
                c4, c5, c6 = st.columns(3)
                c4.metric("Clean Data Participants", len(post_scores))
        ...
        st.markdown("---")
        st.subheader("Participant Score Distribution Comparison" if (pre_df is not None and not pre_df.empty and post_df is not None and not post_df.empty) else "Participant Score Distribution")
        
        if pre_df is not None and not pre_df.empty and post_df is not None and not post_df.empty:
        ...
        if post_df is not None and not post_df.empty:
            ...
            with col_dl2 if pre_df is not None and not pre_df.empty else col_dl1:
                st.download_button(
        ...
        if not api_key:
            ...
            if "generated_insights" not in st.session_state:
                with st.spinner("Analyzing available data and building action plan..."):
                    pre_results_dict = dict(zip(df_pre['Question'], df_pre['Accuracy (%)'])) if pre_df is not None and not pre_df.empty else None
                    post_results_dict = dict(zip(df_post['Question'], df_post['Accuracy (%)'])) if post_df is not None and not post_df.empty else None
        ...
        st.error("After cleaning the data, there were no participants left to analyze. Ensure your dataset contains English responses.")